...
setup_docker_users() {
    ...
    # Create suprclaw user for managing containers
    if ! id "suprclaw" &>/dev/null; then
        useradd -m -s /bin/bash suprclaw
        usermod -aG sudo suprclaw
        usermod -aG docker suprclaw
        log_info "Created suprclaw user"
    else
        usermod -aG docker suprclaw
        log_info "Added suprclaw to docker group"
    fi
}
...
EOF
    ...
    # Set permissions
    chown -R suprclaw:suprclaw "$TRAEFIK_DIR"
...
/var/log/suprclaw/*.log {
    ...
    missingok
    notifempty
    create 0644 suprclaw suprclaw
}
EOF
    
    mkdir -p /var/log/suprclaw
    chown suprclaw:suprclaw /var/log/suprclaw

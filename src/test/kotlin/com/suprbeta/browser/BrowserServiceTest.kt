...
class BrowserServiceTest {
        ...
        try {
                ...
                notificationRepository.createNotification(
                    ...
                    match { payload ->
                        payload["sessionId"]?.toString()?.contains(savedSession.captured.id) == true &&
                            ...
                            payload["taskId"]?.toString()?.contains("task-1") == true
                    },
                    null
                )
            }
            coVerify {
                pushNotificationSender.sendNotification(
                    ...
                    data = match { it["browserState"] == BrowserSessionState.ACTIVE && it["taskId"] == "task-1" },
                    highPriority = true
                )
            }
        } finally {
        ...
        try {
            ...
            coVerify {
                pushNotificationSender.sendNotification(
                    ...
                    body = "Open the live browser viewer in SuprClaw.",
                    data = match { it["browserEventType"] == "browser.takeover.requested" },
                    highPriority = true
                )
            }
        } finally {
    ...
    fun `resume session returns active when provider session still exists`() = testApplication {
        ...
        try {
            ...
            assertEquals(BrowserSessionState.ACTIVE, result.state)
            assertEquals(BrowserSessionState.ACTIVE, saved.captured.state)
            assertEquals("done", saved.captured.lastError)
    ...
    fun `resume session expires and releases lock when provider session is gone`() = testApplication {
        ...
        try {
            ...
            assertEquals(BrowserSessionState.EXPIRED, saved.captured.state)

...
if [ -z "${PICOCLAW_PROVIDER_API_KEY:-}" ] && [ -z "${LITELLM_API_KEY:-}" ] && [ -z "${OPENAI_API_KEY:-}" ]; then
  cat >&2 <<'WARN'
[suprclaw] No model provider API key is configured for this container.
[suprclaw] The PicoClaw gateway will start for provisioning, but inference requests will fail until a provider key is supplied.
WARN
fi
